...
parser$add_argument("--TaxonomyFile",
		         help="Provide Provide Taxonomy file;OTU column name should be gene_id ")
...
taxonomy<-read_delim(args$TaxonomyFile,delim = ",",show_col_types = FALSE)
...
normalized_countsNWithGene<-merge(normalized_countsN, taxonomy,by.x ="Gene_ID",by.y="gene_id",all.x = TRUE)
write.table(normalized_countsNWithGene,file.path(ResultDir,paste0(OutputFilename,"_","ReadCountNormalised_WithTaxonomyDetails.xls")),sep = "\t",row.names = F)
...
RlogNCount_countsNWithGene<-merge(RlogNCount, taxonomy,by.x ="Gene_ID",by.y="gene_id",all.x = TRUE)
write.table(RlogNCount_countsNWithGene,file.path(ResultDir,paste0(OutputFilename,"_","ReadCountRlogNorm_WithTaxonomyDetails.xls")),sep = "\t",row.names = F)
...
res_data_NormReadCountWithGene<-merge(res_data_NormReadCount, taxonomy,by.x ="Geneid",by.y="gene_id",all.x = TRUE)
write.csv(res_data_NormReadCountWithGene,file.path(paste0("DES/Data/",Name),paste0(OutputFilename,"_",Name,"_","DESeq2with_NormReadCount_WithTaxonomyDetails.csv"))) 


row.names(taxonomy)<-taxonomy$gene_id
resWithGene<-merge(data.frame(res), taxonomy,by=0,all.x = TRUE)
resWithGene = subset(resWithGene, select = -c(gene_id) )
colnames(resWithGene)[which(names(resWithGene) == "Row.names")] <- "gene_id"

write.csv(resWithGene,file.path(paste0("DES/Data/",Name),paste0(OutputFilename,"_",Name,"_","DESeq2_WithTaxonomyDetails.csv"))) 
